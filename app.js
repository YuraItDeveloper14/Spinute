...
// Pure helpers live in analysis.js, which the page loads first.
const { escapeHtml, highlightTranscript } = window.SpinuteAnalysis;
...
// ── Local analysis ────────────────────────────────────────────────────────────
function analyze() {
  return SpinuteAnalysis.analyzeSpeech(
    { text: state.transcript, elapsed: state.speakElapsed,
      activity: state.activity, startTs: state.speakStartTs },
    FILLERS[state.lang.key], CONNECTORS[state.lang.key]);
}

function buildTips(a) {
  return SpinuteAnalysis.buildTips(a, { noAnalysis: state.noAnalysis,
                                        connectors: CONNECTORS[state.lang.key] });
}
